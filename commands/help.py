...
class HelpCommand(commands.Cog):
    ...
    async def help_cmd(self, ctx):
        ...
        embed = discord.Embed(title=f"{ice} Storm G3N — Help Panel", color=0x001000)
        ...
        embed.add_field(
            ...
            value=(
                "👑 `$freeadd <service> [filter:no]` → Ajouter un service au Free Vault\n"
                "👑 `$freeremove <service>` → Retirer un service du Free Vault\n"
                "👑 `$boostadd <service> [filter:no]` → Ajouter un service au Booster Vault\n"
                "👑 `$boostremove <service>` → Retirer un service du Booster Vault\n"
        ...
        embed.add_field(
            ...
            value=(
                "`$ticket [raison]` → Ouvrir un ticket de support\n"
                "`$closeticket [motif]` → Fermer le ticket actuel\n"
                "`$ticketadd @user` → Ajouter un membre au ticket *(staff)*\n"
                "`$ticketremove @user` → Retirer un membre du ticket *(staff)*\n"
